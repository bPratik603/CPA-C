    ...
    switch(uMsg)
    {
       ...
       case WM_PAINT:
           ...
           for(i = 0;i< NR_STUDENTS;++i)
           {
                ...
                TextOut(hdc,0,i*cyChar,szRollBuffer,BUFFER_SIZE);
                TextOut(hdc,NR_MAX_CHAR_IN_ROLL * cxChar,i * cyChar,strec[i].szName,lstrlen(strec[i].szName));
                TextOut(hdc,
                        NR_MAX_CHAR_IN_ROLL + NR_MAX_CHAR_IN_ATTENDANCE * cxCaps,
                        i * cyChar,
                        szAttendanceBuffer,
                        lstrlen(szAttendanceBuffer));
                TextOut(hdc,(NR_MAX_CHAR_IN_ROLL + NR_MAX_CHAR_IN_ATTENDANCE) * cxChar + 
