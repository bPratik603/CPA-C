...
#define NR_MAX_CHAR_IN_ATTENDANCE   8 
                    ...
                    int nShowCmd)
{
    ...
    static TCHAR szTitleOfWindow[] = TEXT("Student Information"); 
    ...
    {
        MessageBox(NULL, TEXT("Failed to register a window class"), TEXT("RegisterClassEx"), 
    ...
    if(hwnd == NULL)
    {
        MessageBox(NULL, TEXT("Failed to create an application window"), TEXT("CreateWindowEx"), 
    ...
    switch(uMsg)
    {
        ...
        case WM_PAINT: 
            ...
            for(i = 0; i < NR_STUDENTS; ++i)
            {
                ...
                ZeroMemory(szAttendanceBuffer, BUFFER_SIZE); 
                snprintf(szRollBuffer, BUFFER_SIZE, TEXT("%d"), strec[i].iRollNumber); 
                snprintf(szAttendanceBuffer, BUFFER_SIZE, TEXT("%.1f"), strec[i].fAttendance); 
                TextOut(hdc, 
                        0, 
                        i * cyChar, 
                        szRollBuffer, 
                        BUFFER_SIZE); 
                TextOut(hdc, 
                        NR_MAX_CHAR_IN_ROLL * cxChar, 
                        i * cyChar, 
                         strec[i].szName, 
                        lstrlen(strec[i].szName)
                );
                TextOut(hdc, 
                        NR_MAX_CHAR_IN_ROLL * cxChar + NR_MAX_CHAR_IN_NAME * cxCaps, 
                        i * cyChar, 
                        szAttendanceBuffer, 
                        lstrlen(szAttendanceBuffer)
                    ); 
                TextOut(hdc, 
                        (NR_MAX_CHAR_IN_ROLL + NR_MAX_CHAR_IN_ATTENDANCE) * cxChar + 
                        NR_MAX_CHAR_IN_NAME * cxCaps, 
                        i * cyChar, 
                        strec[i].szCity, 
