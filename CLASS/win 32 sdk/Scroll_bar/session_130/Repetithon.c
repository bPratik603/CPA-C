...
LRESULT CALLBACK WndProc(HWND hwnd,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
    ...
    switch(uMsg)
    {
         case WM_CREATE:
            ...
            iHscrollMin = 0;
            iHscrollMax = 2 + (cxChar * 20 + cyChar * 16) / cxChar;
        ...
        case WM_PAINT:
           hdc = BeginPaint(hwnd,&ps);
            ...
            {
                ZeroMemory(szRollNumberBuffer,BUFFER_SIZE);
                ZeroMemory(szAttendanceBuffer,BUFFER_SIZE);
                snprintf(szRollNumberBuffer,BUFFER_SIZE,TEXT("%d"),st_rec[i].iRollNumber);
                snprintf(szAttendanceBuffer,BUFFER_SIZE,TEXT("%.1f"),st_rec[i].fAttedance);
                TextOut(hdc,
                        cxChar * (1 - iHscrollPosition) + 0,
                ...
                TextOut(hdc,
                        cxChar * (1 - iHscrollPosition) + 5 * cxChar,
                        cyChar * (i - iVscrollPosition),
                        st_rec[i].szStudentName,
                        lstrlen(st_rec[i].szStudentName)
                        );

                TextOut(hdc,
                       cxChar * (1 - iHscrollPosition) + 5 * cxChar + 16 * cxCaps,
                       cyChar * (i - iVscrollPosition),
                       szAttendanceBuffer,
                       BUFFER_SIZE
                       );
                ...
                TextOut(hdc,
                       cxChar * (1 - iHscrollPosition) + 15 * cxChar + 16 * cxCaps,
                       cyChar * (i - iVscrollPosition),
